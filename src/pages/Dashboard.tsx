...
const inputCls = (extra = '') =>
  `w-full bg-white/5 border border-white/10 focus:border-blue-500/40 rounded-xl px-4 py-3 text-white placeholder-white/20 text-sm outline-none transition-all ${extra}`;
const labelCls = () => 'block text-xs text-white/40 uppercase tracking-widest mb-2';
...
export default function Dashboard() {
  ...
  const closeModal = () => {
    setModal(null); setModalData(null); setFormSuccess('');
    setPwError(''); setPwChange({ newPw: '', confirm: '', show: false });
  };
  ...
  const handleCreateUser = (e: React.FormEvent) => {
    ...
    const u: User = {
      ...
      role: newUser.role as 'Admin' | 'Employee', status: 'active', password: newUser.password,
    };
    setUsers(p => [...p, u]);
  ...
  const toggleUserStatus = (id: number) => {
    ...
    if (u) addActivity(`${u.name} marked ${u.status === 'active' ? 'inactive' : 'active'}`);
  };
  ...
  const handleCreateReport = (e: React.FormEvent) => {
    e.preventDefault();
    const r: Report = {
      id: Date.now(), title: newReport.title, author: 'Ryan Mitchell',
      date: new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }),
      severity: newReport.severity as Report['severity'], body: newReport.body,
    };
    setReports(p => [r, ...p]);
    addActivity(`Report filed: ${r.title}`, r.severity === 'critical' ? 'bg-red-400' : r.severity === 'warning' ? 'bg-amber-400' : 'bg-blue-400');
  ...
  const handleCreateSOP = (e: React.FormEvent) => {
    e.preventDefault();
    const s: SOP = {
      id: Date.now(), title: newSOP.title, category: newSOP.category, version: newSOP.version,
  ...
  const handleAddPayslip = (e: React.FormEvent) => {
    e.preventDefault();
    const p: Payslip = {
      id: Date.now(), userId: Number(newPayslip.userId),
      period: newPayslip.period, date: newPayslip.date,
      gross: newPayslip.gross, net: newPayslip.net,
      status: newPayslip.status as 'paid' | 'pending',
    };
  ...
  const runAudit = () => {
    ...
    setTimeout(() => {
      setAuditFindings([
        ...
        { text: `${users.filter(u => u.role === 'Admin').length} admin account(s) found`, ok: users.filter(u => u.role === 'Admin').length <= 2 },
  ...
  return (
    <div className="min-h-screen w-full bg-black text-white flex overflow-hidden">
      ...
      {/* Maintenance banner */}
      {maintenance && (
        <div className="fixed top-0 left-0 right-0 z-50 bg-amber-500/90 text-black text-xs font-bold text-center py-1.5 tracking-widest uppercase">
      ...
      <main className={`flex-1 flex flex-col min-w-0 relative z-10 overflow-hidden ${maintenance ? 'mt-7' : ''}`}>
        <header className="flex items-center justify-between px-6 md:px-8 py-5 border-b border-white/8 backdrop-blur-xl bg-black/20 flex-shrink-0">
          <div className="flex items-center gap-4">
            <button className="md:hidden text-white/40 hover:text-white" onClick={() => setSidebarOpen(v => !v)}>
            ...
            <div className="space-y-8">
              ...
              {view === 'admin' ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <StatCard label="Reports" value={String(reports.length)} sub={`${openReports} need attention`} color="text-amber-400" />
                  ...
                  {users.map(user => (
                    <div key={user.id} className="flex items-center justify-between px-6 py-4 hover:bg-white/[0.02] transition-colors">
                      ...
                      <div className="flex items-center gap-3 ml-4 flex-shrink-0">
                        ...
                        <button onClick={() => toggleUserStatus(user.id)} title={`Toggle status (currently ${user.status})`}
                          className={`w-2.5 h-2.5 rounded-full transition-all hover:scale-150 cursor-pointer ${user.status === 'active' ? 'bg-emerald-400' : 'bg-white/20'}`} />
                    ...
                    return (
                      <div key={slip.id} className="flex items-center justify-between px-6 py-4 hover:bg-white/[0.02] transition-colors">
                        ...
                        <div className="flex items-center gap-4 ml-4">
                          ...
                          <button onClick={() => downloadPayslip(slip)} className="p-2 rounded-lg text-white/25 hover:text-violet-400 hover:bg-violet-500/10 transition-all" title="Download">
                            <Download className="w-4 h-4" />
                          </button>
                          {view === 'admin' && (
                            <button onClick={() => { setPayslips(p => p.filter(x => x.id !== slip.id)); addActivity(`Payslip removed: ${slip.period}`, 'bg-red-400'); }}
                              className="p-2 rounded-lg text-white/25 hover:text-red-400 hover:bg-red-500/10 transition-all" title="Delete">
                  ...
                  <GlassCard key={report.id} className="p-5 hover:bg-white/[0.05] transition-all duration-200">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <button onClick={() => { setModal('view-report'); setModalData(report); }} className="p-2 rounded-lg text-white/25 hover:text-white/60 hover:bg-white/5 transition-all">
                          <Eye className="w-4 h-4" />
                        </button>
                        {view === 'admin' && (
                          <button onClick={() => { setModal('delete-report'); setModalData(report); }} className="p-2 rounded-lg text-white/25 hover:text-red-400 hover:bg-red-500/10 transition-all">
                            <Trash2 className="w-4 h-4" />
                          </button>
                        )}
                      </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {[
                  ...
                  {
                    icon: maintenance ? <ToggleRight className="w-5 h-5 text-amber-400" /> : <ToggleLeft className="w-5 h-5 text-white/40" />,
                    title: 'Maintenance Mode', desc: maintenance ? 'Site is in maintenance' : 'Toggle site maintenance',
                    action: maintenance ? 'Disable' : 'Enable', color: maintenance ? 'amber' : 'white',
                    onClick: () => { setMaintenance(v => !v); addActivity(maintenance ? 'Maintenance disabled' : 'Maintenance enabled', 'bg-amber-400'); },
                  },
                  ...
                  <GlassCard key={i} className="p-6 hover:bg-white/[0.06] transition-all duration-200">
          ...
          {formSuccess
            ...
            : <form onSubmit={handleChangePassword} className="space-y-4">
                ...
                <div className="flex justify-end gap-3 pt-2">
                  ...
                  <button type="submit" className="px-5 py-2.5 rounded-xl text-sm font-semibold text-white bg-blue-500/20 border border-blue-500/30 hover:bg-blue-500/30 transition-all">Update Password</button>
                </div>
              </form>
          }
        </ModalWrap>
      ...
      {modal === 'audit' && (
        <ModalWrap title="Security Audit" onClose={closeModal}>
          {auditStatus === 'scanning'
            ? <div className="flex flex-col items-center gap-4 py-8"><RefreshCw className="w-8 h-8 text-blue-400 animate-spin" /><p className="text-sm text-white/50">Scanning platform…</p></div>
            : <div className="space-y-3">
                <p className="text-xs text-white/40 mb-4">Completed · {new Date().toLocaleString()}</p>
      ...
      {modal === 'health-check' && (
        <ModalWrap title="Health Check" onClose={() => { closeModal(); setHealthStatus('idle'); }}>
          {healthStatus === 'checking'
            ? <div className="flex flex-col items-center gap-4 py-8"><Wifi className="w-8 h-8 text-cyan-400 animate-pulse" /><p className="text-sm text-white/50">Checking all services…</p></div>
            : <div className="space-y-3">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-white/5">
                {[
                  ['View Dashboard', true, true], ['Manage Users', true, false],
                  ['Create Reports', true, true], ['Delete Reports', true, false],
                  ['Manage SOPs', true, false], ['View SOPs', true, true],
                  ['Manage Payslips', true, false], ['View Own Payslips', true, true],
                  ['Site Tools Access', true, false], ['Export Data', true, false],
